...
<div class="container w-100 d-flex flex-column align-items-center">
    <div class="d-flex flex-column align-items-center">
        <p class="topic poppins-bold m-0">Tambah Image Portfolio</p>
    </div>
    <form method="POST" class="w-50" action="{{ url('/admin/PortfolioImage') . '/' . $portfolio->id . '/' . 'add' }}" enctype="multipart/form-data">
        @csrf

        <input id="portfolio_id" type="hidden" class="form-control" name="portfolio_id" value="{{ $portfolio->id }}">
        <input id="portfolio_name" type="hidden" class="form-control" name="portfolio_name" value="{{ $portfolio->name }}">
        ...
        <div class="form-group modal-btn row justify-content-end align-items-end mb-0">
            ...
            <div class="col-2">
                <a href="{{ url('/admin/portfolio') . '/' . $portfolio->id }}"><button type="button" class="btn btn-danger" class="close">Cancel</button></a>
            </div>
        </div>
    </form>
</div>
