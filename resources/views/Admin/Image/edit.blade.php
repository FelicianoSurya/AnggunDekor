...
<div class="container w-100 d-flex flex-column align-items-center">
    <div class="d-flex flex-column align-items-center">
        <p class="topic poppins-bold m-0">Edit Image Product</p>
    </div>
    <form method="POST" class="w-50" action="{{ url('/admin/productImage') . '/' . 'edit' }}" enctype="multipart/form-data">
        @csrf

        <input id="product_id" type="hidden" class="form-control" name="product_id" value="{{ $data->id }}">
        <input id="image_id" type="hidden" class="form-control" name="image_id" value="{{ $image->id }}">
        <input id="product_name" type="hidden" class="form-control" name="product_name" value="{{ $data->name }}">
        ...
        <div class="form-group modal-btn row justify-content-end align-items-end mb-0">
            ...
            <div class="col-2">
                <a href="{{ url('/admin/products') . '/' . $data->id }}"><button type="button" class="btn btn-danger" class="close">Cancel</button></a>
            </div>
        </div>
    </form>
</div>
